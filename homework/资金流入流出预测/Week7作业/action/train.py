...
class TrainPipeline():
    def __init__(self, init_model=None, use_gpu=False):
        ...
        self.game_batch_num = 300 # 训练多少个epoch
    ...
    def policy_evaluate(self, n_games=20):
        ...
        self.log_record += "num_playouts:{}, win: {}, lose: {}, tie:{}\n\n".format(
                self.pure_mcts_playout_num,
                win_cnt[1], win_cnt[2], win_cnt[-1])
        return win_ratio
    ...
    def run(self):
        ...
        try:
            ...
            for i in range(self.game_batch_num):
                ...
                self.log_record += "batch i:{}, episode_len:{}\n".format(i+1, self.episode_len)
                if len(self.data_buffer) > self.batch_size:
                    loss, entropy = self.policy_update()
                # 判断当前模型的表现，保存最优模型
                if (i+1) % self.check_freq == 0:
                    print("current self-play batch: {}".format(i+1))
                    self.log_record += "current self-play batch: {}".format(i+1)
                    win_ratio = self.policy_evaluate()
                    # 保存当前策略
                    self.policy_value_net.save_model('./current_policy.model')
                    if win_ratio > self.best_win_ratio:
                        print("发现新的最优策略，进行策略更新")
                        self.log_record += str(datetime.now())[:19] + f"发现新的最优策略，胜率为{win_ratio}，进行策略更新"
                        self.best_win_ratio = win_ratio
                        # 更新最优策略
                        self.policy_value_net.save_model(f'./best_policy_{win_ratio:.2f}.model')
                        if (self.best_win_ratio == 1.0 and
                                self.pure_mcts_playout_num < 5000):
                            self.pure_mcts_playout_num += 1000
                            self.best_win_ratio = 0.0
                with open('./log/log.txt', 'a+') as f:
                    f.write(self.log_record)
                    self.log_record = ''
